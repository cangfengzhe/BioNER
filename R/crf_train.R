...
#' @useDynLib BioTmR
#' 
crf_train <- function( train_data, template_file, model_dir = getwd(), freq = 1,  max_iterations = 10000, cost = 1,  eta =0.0001, algorithm = 'CRF-L2',  shrinking_size = 20, thread = 0){
 ...
 algorithm <- match.arg(algorithm, c('CRF-L2', 'CRF-L1','mira'))
  ...
  crf_learn_(template_file,  train_path,  model_path, freq,  max_iterations, cost,  eta, algorithm,  shrinking_size)
