...
// ifilename = "/Users/lipidong/Desktop/genia.txt";
  ...
  for (int i = 0; i < 8; i +=2 ) {
    char buf[1000];
    //sprintf(buf, "/Users/lipidong/baiduyun/work/RFile/genia/src/models_chunking/model.bidir.%d", i);
...
vector<string> v222 = resplit(line, "[\\.?!]\\s(?=[A-Z])");
     ...
     for (const auto & e: v222) {
       ...
       if (e.size() > 1024) {
         ...
         cerr << " (please note that every sentence should be splited by '. ')." << endl;
       }
       string postagged = bidir_postag(e, vme, vme_chunking, dont_tokenize);
       //cout << postagged << endl;
